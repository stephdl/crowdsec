...
# this may break things if all possible errors are not taken into account.
set -e
...
crowdsec_service_disable() {
    if command -v systemctl >/dev/null && systemctl is-enabled --quiet crowdsec; then
        systemctl disable crowdsec.service >/dev/null
    fi
}
...
install_crowdsec() {
    ...
    install -m 600 "./config/$CLIENT_SECRETS" "$CROWDSEC_CONFIG_PATH"
    install -m 600 "./config/$LAPI_SECRETS" "$CROWDSEC_CONFIG_PATH"

    install -m 600 ./config/config.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/dev.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/user.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/acquis.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/profiles.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/simulation.yaml "$CROWDSEC_CONFIG_PATH"
    install -m 644 ./config/console.yaml "$CROWDSEC_CONFIG_PATH"

    mkdir -p "$CROWDSEC_CONFIG_PATH/hub"
    install -m 644 ./config/detect.yaml "${CROWDSEC_CONFIG_PATH}/hub"
...
install_bins() {
    ...
    install -m 755 "$CROWDSEC_BIN" "$CROWDSEC_BIN_INSTALLED" >/dev/null
    install -m 755 "$CSCLI_BIN" "$CSCLI_BIN_INSTALLED" >/dev/null
...
detect_edit_validate() {
	...
	# blah blah blah
	#
	# Out of safety, we recomment installing the collection 'crowdsecurity/whitelists'.
	# It will prevent private IP addresses from being banned, feel free to remove it any time.
	#
	...
	EOT
        ...
        printf "Crowdsec has detected these services. Do you want to edit the list now? (Y/n) "
        read -r confirm
        ...
        while true; do
            ...
            if ! errors=$("$CSCLI_BIN_INSTALLED" setup validate <"$setup_yaml_path" 2>/dev/null); then
                ...
                if [ "$errors" = "EOF" ]; then
                    errors="The file is empty. A 'setup:' section is required (even if it's empty)."
                fi

                echo "$errors"
                echo
                printf '[E]dit, [D]etect again, [Q]uit configuration? (E/d/q) '
...
detect_and_install_collections() {
    tmp_dir=$(mktemp -d)
...
main() {
    ...
    if [ "$1" = "configure" ]; then
        ...
        "$CSCLI_BIN_INSTALLED" hub update
        detect_and_install_collections
        # XXX should restart systemctl and show links again
        return 0
    fi
...
usage() {
    ...
    echo "    ./wizard.sh -c|--configure                   Detect running services and install collections + acquis files"
